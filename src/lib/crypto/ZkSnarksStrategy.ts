import { ICryptoStrategy } from "./CryptoStrategy"
...
export type ZkSnarkOptions = {
   initializeClient?: boolean
   uuid: string
   sharedSecret: string
}

const ZkSnarksDefaultOptions: ZkSnarkOptions = {
   initializeClient: false
}

export class ZkSnarkCryptoStrategy implements ICryptoStrategy {
   readonly options: ZkSnarkOptions

   private provider: zokrates.ZoKratesProvider | undefined = undefined
   private hasInit: boolean = false

   constructor(options: ZkSnarkOptions) {
      this.options = Object.assign({}, ZkSnarksDefaultOptions, options)

      if (options.uuid == null) {
         throw new Error(`Must provide a uuid when using the ZkSnarksCryptoStrategy`)
      }

      if (options.sharedSecret == null) {
         throw new Error(`Must provide a sharedSecret when using the ZkSnarksCryptoStrategy`)
      }
   }
   ...
   init(): Promise<void> {
      ...
      if (this.options.initializeClient) {
   ...
   private stringToNumberChunks(v: string) {
      ...
      for (let i = 0, j = numbers.length; i < j; i += chunksize) {
         let temporary = numbers.slice(i, i + chunksize);
